...
async def startup_event():
    ...
    try:
        ...
        db = load_existing_db()
        logger.info("Startup complete. DB loaded: %s", db is not None)
    except Exception as exc:
        logger.warning("Could not load existing DB on startup: %s", exc)
...
async def upload_pdf(files: list[UploadFile] = File(...)):
    """Upload PDFs and ingest into ChromaDB."""
    global db
    try:
...
def ask(request: AskRequest):
    """Answer a question against the ChromaDB collection."""
    if db is None:
        return {
